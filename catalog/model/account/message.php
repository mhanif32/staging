...
class ModelAccountMessage extends Model {
	public function addMessage($data) {
		$this->db->query("INSERT INTO ". DB_PREFIX ."customer_message SET mpseller_id = '". (int)$data['mpseller_id'] ."', `from` = '". $this->db->escape($data['from']) ."', message = '". $this->db->escape($data['message']) ."', date_added = NOW()");
	...
	public function getTotalUnreadMessages($mpseller_id) {
		$query = $this->db->query("SELECT COUNT(*) as total FROM " . DB_PREFIX . "customer_message WHERE mpseller_id = '" . (int)$mpseller_id . "' AND `from` = 'admin' AND read_status = '0'");

		return $query->row['total'];
	}

	public function MarkAsReadMessage($mpseller_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer_message SET  read_status = '1' WHERE mpseller_id = '" . (int)$mpseller_id . "' AND `from` = 'admin'");
	}

	public function getTotalSellerMessageChats($data = array()) {
		$sql = "SELECT COUNT(*) as total FROM " . DB_PREFIX . "customer_message WHERE mpseller_id = '". (int)$data['mpseller_id'] ."'";
	...
	public function getSellerMessageChats($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "customer_message WHERE mpseller_id = '". (int)$data['mpseller_id'] ."' ORDER BY customer_message_id ASC";
