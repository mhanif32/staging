...
class ModelAccountMpmultivendorProduct extends Model {
	public function addProduct($data, $mpseller_id) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$value['description'] = strip_tags(html_entity_decode($value['description'], ENT_QUOTES, 'UTF-8'));
		...
		if (isset($data['product_download'])) {
			foreach ($data['product_download'] as $download_id) {
				$download_info = $this->getDownload($download_id, $mpseller_id);
...
//		}
	...
	public function editProduct($product_id, $data, $mpseller_id) {
        ...
        if(!empty($data['currency'])) {
            if($data['currency'] == 'USD') {
                ...
                //echo '<pre>';print_r($dataValue);echo '<pre>';exit('ygyg');
            }
        } else {
            $data['price'] = $data['local_price'];
        }

        //echo '<pre>';print_r($data['price']);exit('okok');
    ...
    public function getSpecialProducts($data = array()) {
        $sql = "SELECT * FROM " . DB_PREFIX . "product p RIGHT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) 
        RIGHT JOIN " . DB_PREFIX . "product_special ps ON (p.product_id = ps.product_id) 
    ...
    public function getTotalSpecialProducts($data = array()) {
        $sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "product p RIGHT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)
        RIGHT JOIN " . DB_PREFIX . "product_special ps ON (p.product_id = ps.product_id)";
	...
	public function getProductBySellerID($product_id, $mpseller_id) {
		$query = $this->db->query("SELECT * FROM ". DB_PREFIX ."product WHERE product_id = '". (int)$product_id ."' AND mpseller_id = '". (int)$mpseller_id ."'");
	...
	public function getDownload($download_id, $mpseller_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "download d LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id) WHERE d.download_id = '" . (int)$download_id . "' AND dd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND d.mpseller_id = '". (int)$mpseller_id ."'");

		return $query->row;
	}

	public function getProduct($product_id, $mpseller_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.mpseller_id = '". (int)$mpseller_id ."'");
