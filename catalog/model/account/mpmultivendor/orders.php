...
class ModelAccountMpmultivendorOrders extends Model {
    ...
    public function getPendingOrders($data = array()) {
        $sql = "SELECT mpo.order_id, o.firstname, o.lastname, os.name as status, os.order_status_id, osadmin.name as by_admin_status, o.date_added, o.currency_code, o.currency_value, (SELECT `value` FROM ". DB_PREFIX ."mpseller_order_total mpot WHERE mpot.order_id = mpo.order_id AND mpot.mpseller_id = '" . (int)$data['mpseller_id'] . "' AND `code` = 'sub_total') AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN `". DB_PREFIX ."mpseller_order_product` mpo ON (o.order_id=mpo.order_id) LEFT JOIN " . DB_PREFIX . "order_status os ON (mpo.order_status_id = os.order_status_id) LEFT JOIN " . DB_PREFIX . "order_status osadmin ON (o.order_status_id = osadmin.order_status_id) WHERE mpo.mpseller_id = '" . (int)$data['mpseller_id'] . "' AND os.order_status_id != '5' AND mpo.order_status_id > '0' AND o.store_id = '" . (int)$this->config->get('config_store_id') . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "'";
