<?php
class ModelAccountDeliverypartnerMessage extends Model {
	public function addMessage($data) {
		$this->db->query("INSERT INTO ". DB_PREFIX ."delivery_partner_message SET delivery_partner_id = '". (int)$data['delivery_partner_id'] ."', `from` = '". $this->db->escape($data['from']) ."', message = '". $this->db->escape($data['message']) ."', subject = '". $this->db->escape($data['subject']) ."', date_added = NOW()");
		...
		$seller_info = $this->model_mpmultivendor_mv_seller->getSellerStoreInfo($data['delivery_partner_id']);
	...
	public function getTotalUnreadMessages($delivery_partner_id) {
		$query = $this->db->query("SELECT COUNT(*) as total FROM " . DB_PREFIX . "delivery_partner_message WHERE delivery_partner_id = '" . (int)$delivery_partner_id . "' AND `from` = 'admin' AND read_status = '0'");

		return $query->row['total'];
	}

	public function MarkAsReadMessage($delivery_partner_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "delivery_partner_message SET  read_status = '1' WHERE delivery_partner_id = '" . (int)$delivery_partner_id . "' AND `from` = 'admin'");
	}
	...
	public function getDeliveryPartnerMessageChats($data = array()) {
