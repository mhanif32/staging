<?php
class ModelAccountEnquiries extends Model {
	...
	public function getEnquiry($customer_id, $enquiry_id) {
		$sql = "SELECT * FROM " . DB_PREFIX . "mpseller_enquiry WHERE mpseller_enquiry_id > 0";
	...
	public function getEnquiryChats($customer_id, $enquiry_id, $start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$sql = "SELECT c.*, e.name FROM " . DB_PREFIX . "mpseller_enquiry_chat c LEFT JOIN " . DB_PREFIX . "mpseller_enquiry e ON(c.mpseller_enquiry_id = e.mpseller_enquiry_id) WHERE e.customer_id = '" . (int)$customer_id . "' AND e.mpseller_enquiry_id = '" . (int)$enquiry_id . "' ORDER BY c.mpseller_enquiry_chat_id ASC LIMIT " . (int)$start . "," . (int)$limit;
	...
	public function getTotalEnquiryChats($customer_id, $enquiry_id) {
		$sql = "SELECT COUNT(*) as total FROM " . DB_PREFIX . "mpseller_enquiry_chat c LEFT JOIN " . DB_PREFIX . "mpseller_enquiry e ON(c.mpseller_enquiry_id = e.mpseller_enquiry_id) WHERE e.customer_id = '" . (int)$customer_id . "' AND e.mpseller_enquiry_id = '" . (int)$enquiry_id . "'";
	...
	public function addEnquiryChat($data) {
		...
		// customer add reply, send mail to seller
		$this->load->model('account/mpmultivendor/orders');
		$seller_info = $this->model_account_mpmultivendor_orders->getMpseller((int)$data['mpseller_id']);
		...
		if ($seller_info) {
			...
			$this->load->language('mpmultivendor_mail/chat_reply_seller_mail');

			$this->load->model('tool/image');

			$subject = sprintf($this->language->get('text_mail_subject'), $data['customer_name'], $data['enquiry_id']);
			$sender = sprintf($this->language->get('text_mail_sender'), $data['customer_name'], $this->config->get('config_name'));
			...
			$mail_data['text_mail_name'] = $data['customer_name'];

			$mail_data['date_added'] = date('M d, h:i');

			$mail_data['message'] = $data['message'];

			$link = $this->url->link('account/mpmultivendor/enquiries/view', 'enquiry_id='. $data['enquiry_id'], true);
			...
			$message = $this->load->view('mpmultivendor_mail/chat_reply_seller_mail', $mail_data);
