...
class ControllerExecutivememberSubscription extends Controller
{
    public function index()
    {
        ...
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('executivemember/subscription', '', true);

            $this->response->redirect($this->url->link('account/login', '', true));
        }

        $this->load->language('executivemember/subscription');
        $this->load->model('executivemember/subscription');
        $this->load->model('account/customer');

        $customer = $this->model_account_customer->getStripeCustomerId($this->customer->getId());

        if (empty($customer['role']) || $customer['role'] != 'executive-member') {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('executivemember/subscription', '', true)
        );
        ...
        //plans
        $planList = $this->model_executivemember_subscription->getSubscriptionPlans($this->session->data['currency']);
        ...
        $data['action_subscribe'] = $this->url->link('executivemember/subscription/add', '', true);
        ...
        $this->response->setOutput($this->load->view('executivemember/subscription', $data));
    }
