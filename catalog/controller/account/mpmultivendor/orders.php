<?php
class ControllerAccountMpmultivendorOrders extends Controller {
	public function index() {
		...
		$order_total = $this->model_account_mpmultivendor_orders->getTotalOrders($filter_data);
		...
		if(VERSION < '2.2.0.0') {
		   ...
		   $this->response->setOutput($this->load->view('account/mpmultivendor/orders', $data));
		}
	}
	...
	public function info() {
		...
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/mpmultivendor/orders/info', 'order_id=' . $order_id, true);

			$this->response->redirect($this->url->link('account/login', '', true));
		}

		$order_info = $this->model_account_mpmultivendor_orders->getOrder($order_id, $mpseller_id);
		...
		if ($order_info) {
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('account/mpmultivendor/orders/info', 'order_id=' . $this->request->get['order_id'] . $url, true)
			);

			$data['cancel'] = $this->url->link('account/mpmultivendor/orders', '', true);
			...
			$data['invoice'] = $this->url->link('account/mpmultivendor/orders/invoice', '&order_id=' . (int)$this->request->get['order_id'], true);
			$data['shipping'] = $this->url->link('account/mpmultivendor/orders/shipping', '&order_id=' . (int)$this->request->get['order_id'], true);
			...
			$products = $this->model_account_mpmultivendor_orders->getOrderProducts($this->request->get['order_id'], $mpseller_id);

			foreach ($products as $product) {
				$option_data = array();

				$options = $this->model_account_mpmultivendor_orders->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
			...
			$totals = $this->model_account_mpmultivendor_orders->getOrderTotals($this->request->get['order_id'], $mpseller_id);
			...
			if(in_array($order_info['order_status_id'], $this->config->get('mpmultivendor_restrict_orderstatus'))) {
				$data['order_change_permission'] = false;
			} else {
				$data['order_change_permission'] = true;
			}
			...
			if(VERSION < '2.2.0.0') {
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/mpmultivendor/order_info.tpl')) {
			    	$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/mpmultivendor/order_info.tpl', $data));
			   } else {
			   		$this->response->setOutput($this->load->view('default/template/account/mpmultivendor/order_info.tpl', $data));
			   }
		  	} else {
			   $this->response->setOutput($this->load->view('account/mpmultivendor/order_info', $data));
			}
		} else {
			$this->document->setTitle($this->language->get('text_order'));
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('account/mpmultivendor/orders', '', true)
			);

			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_order'),
				'href' => $this->url->link('account/mpmultivendor/orders/info', 'order_id=' . $order_id, true)
			);
	...
	public function history() {
		...
		$results = $this->model_account_mpmultivendor_orders->getOrderHistories($this->request->get['order_id'], $mpseller_id);
		...
		if(VERSION < '2.2.0.0') {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/mpmultivendor/order_history.tpl')) {
		    	$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/mpmultivendor/order_history.tpl', $data));
		   } else {
		   		$this->response->setOutput($this->load->view('default/template/account/mpmultivendor/order_history.tpl', $data));
		   }
	  	} else {
		   $this->response->setOutput($this->load->view('account/mpmultivendor/order_history', $data));
		}
	}
	...
	public function addHistory() {
		...
		if (!$json) {
			if ($order_info) {
				$this->model_account_mpmultivendor_orders->addOrderHistory($order_id, $this->request->post['order_status_id'], $this->request->post['comment'], $this->request->post['notify'], $this->request->post['override']);
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$products = $this->model_account_mpmultivendor_orders->getOrderProducts($order_id, $mpseller_id);

				foreach ($products as $product) {
					$option_data = array();

					$options = $this->model_account_mpmultivendor_orders->getOrderOptions($order_id, $product['order_product_id']);
				...
				$totals = $this->model_account_mpmultivendor_orders->getOrderTotals($order_id, $mpseller_id);
		...
		$this->response->setOutput($this->load->view('account/mpmultivendor/order_invoice', $data));
	}
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info && $order_info['shipping_code']) {
				...
				foreach ($products as $product) {
					...
					$product_info = $this->model_account_mpmultivendor_orders->getProduct($product['product_id'], $order_info['language_id']);
					...
					if ($product_info) {
						...
						foreach ($options as $option) {
							...
							$product_option_value_info = $this->model_account_mpmultivendor_orders->getProductOptionValue($product['product_id'], $option['product_option_value_id'], $order_info['language_id']);
