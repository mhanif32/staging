...
class ControllerSubscriptionExecutiveMemberPlan extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $this->load->language('subscription/executive_member_plan');

        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('subscription/executive_member_plan');
    ...
    public function add()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->url->link('subscription/executive_member_plan', 'user_token=' . $this->session->data['user_token'] . $url, true));
        }
    ...
    protected function getList()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('subscription/executive_member_plan', 'user_token=' . $this->session->data['user_token'] . $url, true)
        );

        $data['add'] = $this->url->link('subscription/executive_member_plan/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $data['delete'] = $this->url->link('subscription/executive_member_plan/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        $plan_total = $this->model_subscription_executive_member_plan->getTotalSubscriptionPlans();

        $results = $this->model_subscription_executive_member_plan->getSubscriptionPlans($filter_data);
        ...
        foreach ($results as $result) {
            ...
            $data['plans'][] = array(
                ...
                'edit' => $this->url->link('subscription/executive_member_plan/edit', 'user_token=' . $this->session->data['user_token'] . '&plan_id=' . $result['plan_id'] . $url, true)
            );
        }
        ...
        $data['sort_name'] = $this->url->link('subscription/executive_member_plan', 'user_token=' . $this->session->data['user_token'] . '&sort=cgd.name' . $url, true);
        $data['sort_sort_order'] = $this->url->link('subscription/executive_member_plan', 'user_token=' . $this->session->data['user_token'] . '&sort=cg.sort_order' . $url, true);
        ...
        $pagination->url = $this->url->link('subscription/executive_member_plan', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('subscription/em_plan_list', $data));
    }
    ...
    protected function getForm()
    {
        ...
        if (!isset($this->request->get['plan_id'])) {
            $data['action'] = $this->url->link('subscription/executive_member_plan/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
        } else {
            $data['action'] = $this->url->link('subscription/executive_member_plan/edit', 'user_token=' . $this->session->data['user_token'] . '&plan_id=' . $this->request->get['plan_id'] . $url, true);
        }

        $data['cancel'] = $this->url->link('subscription/executive_member_plan', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        if(!isset($this->request->get['plan_id'])) {
            $this->response->setOutput($this->load->view('subscription/em_plan_form', $data));
        } else {
            $this->response->setOutput($this->load->view('subscription/em_plan_form_edit', $data));
        }
    }

    protected function validateForm()
    {
        if (!$this->user->hasPermission('modify', 'subscription/executive_member_plan')) {
